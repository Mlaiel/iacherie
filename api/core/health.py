...
class HealthCheckResult:
    """Result of individual health check."""    component_name: str
...
class SystemHealthStatus:
    """Overall system health status."""    overall_status: HealthStatus
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for API response."""        return {
    ...
    def is_healthy(self) -> bool:
        """Check if system is healthy."""        return self.overall_status == HealthStatus.HEALTHY
    
    @property
    def unhealthy_components(self) -> List[HealthCheckResult]:
        """Get list of unhealthy components."""        return [
    ...
    def degraded_components(self) -> List[HealthCheckResult]:
        """Get list of degraded components."""        return [
            result for result in self.component_results
...
class IHealthCheck(ABC):
    ...
    @abstractmethod
    def name(self) -> str:
        """Health check name."""        pass
    
    @property
    @abstractmethod
    def component_type(self) -> ComponentType:
        """Component type being checked."""        pass
    
    @abstractmethod
    async def check_health(self) -> HealthCheckResult:
        """Execute health check and return result."""        pass
    
    @property
    def timeout_seconds(self) -> float:
        """Health check timeout in seconds."""        return 5.0
    
    @property
    def tags(self) -> Dict[str, str]:
        """Additional tags for the health check."""        return {}
...
class DatabaseHealthCheck(IHealthCheck):
    ...
    async def check_health(self) -> HealthCheckResult:
        """Check database connectivity and responsiveness."""        start_time = time.perf_counter()
...
class RedisHealthCheck(IHealthCheck):
    ...
    async def check_health(self) -> HealthCheckResult:
        """Check Redis connectivity and responsiveness."""        start_time = time.perf_counter()
...
class ExternalAPIHealthCheck(IHealthCheck):
    ...
    async def check_health(self) -> HealthCheckResult:
        """Check external API availability."""        start_time = time.perf_counter()
...
class StorageHealthCheck(IHealthCheck):
    ...
    async def check_health(self) -> HealthCheckResult:
        """Check storage system availability."""        start_time = time.perf_counter()
...
class HealthCheckManager:
    ...
    async def check_health(self, component_name: Optional[str] = None) -> SystemHealthStatus:
        """Execute health checks and return system status."""        if component_name:
    ...
    async def _execute_health_check(self, health_check: IHealthCheck) -> HealthCheckResult:
        """Execute single health check with timeout."""        try:
    ...
    def _calculate_overall_status(self, results: List[HealthCheckResult]) -> HealthStatus:
        """Calculate overall system status from component results."""        if not results:
    ...
    def _get_uptime_seconds(self) -> float:
        """Get application uptime in seconds."""        return (datetime.now(timezone.utc) - self.start_time).total_seconds()
    
    def get_registered_checks(self) -> List[str]:
        """Get list of registered health check names."""        with self._lock:
...
class SimpleHealthCheck(IHealthCheck):
    ...
    async def check_health(self) -> HealthCheckResult:
        """Always return healthy status."""        return HealthCheckResult(
            component_name=self.name,
            component_type=self.component_type,
            status=HealthStatus.HEALTHY,
...
def get_health_manager() -> HealthCheckManager:
    """Get global health check manager."""    return _health_manager


def register_health_check(health_check: IHealthCheck) -> None:
    """Register a health check with global manager."""    _health_manager.register_health_check(health_check)


async def check_system_health() -> SystemHealthStatus:
    """Check overall system health."""    return await _health_manager.check_health()


async def check_component_health(component_name: str) -> SystemHealthStatus:
    """Check specific component health."""    return await _health_manager.check_health(component_name)
