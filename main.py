...
async def initialize_services():
    ...
    if activation_stats['total'] > 0:
        taux = (activation_stats['already_active'] + activation_stats['activated']) / activation_stats['total'] * 100
        logger.info(f"✅ Services activation: {activation_stats['already_active']} already active + {activation_stats['activated']} newly activated = {activation_stats['already_active'] + activation_stats['activated']}/{activation_stats['total']} ({taux:.1f}%)")
    else:
        logger.info(f"✅ Services activation: Mode minimal (0 microservices)")
