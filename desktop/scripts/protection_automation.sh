...
show_header() {
    echo -e "${RED}${BOLD}"
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║                🛡️ IACHERIE PROTECTION AUTOMATION                 ║"
    echo "║                                                                  ║"
    ...
    case "$watermark_type" in
        "visible")
            ...
            if command -v espeak &> /dev/null; then
                espeak -s 120 -p 30 -a 20 "Protected by IA Chérie. $watermark_text" -w "$watermark_audio" 2>/dev/null || {
    ...
    case "$watermark_type" in
        "visible")
            ...
            convert "$input_file" \
                ...
                -gravity SouthEast -annotate +10+10 "© IA Chérie - $watermark_text" \
...
apply_video_watermark() {
    ...
    case "$watermark_type" in
        "visible")
            ...
            ffmpeg -i "$input_file" \
                -vf "drawtext=text='© IA Chérie - $watermark_text':fontcolor=white@0.5:fontsize=20:x=w-tw-10:y=h-th-10" \
...
apply_text_watermark() {
    ...
    case "$watermark_type" in
        "visible")
            ...
            {
                echo "---"
                echo "© Protected by IA Chérie - $watermark_text"
...
EOF
    ...
    {
        echo "-----BEGIN IACHERIE PROTECTION CERTIFICATE-----"
        ...
        echo "-----END IACHERIE PROTECTION CERTIFICATE-----"
    } > "$certificate_file"
...
From: Fahed Mlaiel / IA Chérie Protection System
Date: $(date '+%B %d, %Y')
...
This notice was generated by the IA Chérie Protection Automation System.
Protection ID: $protection_id
Verification Hash: $(echo "$protection_id$(date)" | sha256sum | cut -d' ' -f1)

---
© 2025 IA Chérie Protection System - All Rights Reserved
EOF
...
main() {
    ...
    log "INFO" "🛡️ Starting IA Chérie Protection Automation"
