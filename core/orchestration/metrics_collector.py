...
class MetricType(Enum):
    """Metric type classification."""    COUNTER = "counter"
    GAUGE = "gauge"
    HISTOGRAM = "histogram"
    TIMER = "timer"
    RATE = "rate"
...
class MetricDefinition:
    """Metric definition and configuration."""    metric_id: str
    name: str
...
@dataclass
class MetricValue:
    """Individual metric measurement."""    metric_id: str
...
class AggregatedMetric:
    """Aggregated metric result."""    metric_id: str
...
@dataclass
class Dashboard:
    """Metrics dashboard configuration."""    dashboard_id: str
    name: str
    description: str
...
class MetricsCollector:
    ...
    def _start_background_tasks(self) -> None:
        """Start background metrics processing tasks."""        asyncio.create_task(self._aggregation_task())
    ...
    async def _auto_register_metric(self, metric_id: str, value: Union[int, float]) -> None:
        """Auto-register metric with default configuration."""        metric_type = MetricType.GAUGE if isinstance(value, float) else MetricType.COUNTER
    ...
    async def _register_alert_rule_from_config(self, metric_id: str, config: Dict[str, Any]) -> None:
        """Register alert rule from configuration."""        rule = AlertRule(
    ...
    def _tags_match(self, metric_tags: Dict[str, str], filter_tags: Dict[str, str]) -> bool:
        """Check if metric tags match filter tags."""        for key, value in filter_tags.items():
    ...
    async def _aggregate_metric(self, metric_id: str, interval: int, end_time: datetime) -> None:
        """Aggregate metric for specific interval."""        try:
    ...
    async def _alert_evaluation_task(self) -> None:
        """Background task for alert evaluation."""        while True:
            try:
                current_time = datetime.now()
    ...
    async def _trigger_alert(self, rule: AlertRule, metric_value: Union[int, float], timestamp: datetime) -> None:
        """Trigger new alert."""        alert_id = str(uuid.uuid4())
    ...
    async def _performance_monitoring_task(self) -> None:
        """Background task for performance monitoring."""        last_metric_count = 0
    ...
    async def _get_widget_data(self, widget_config: Dict[str, Any]) -> Dict[str, Any]:
        """Get data for dashboard widget."""        widget_type = widget_config.get('type', 'line_chart')
    ...
    async def _analyze_trends(self, values: List[Union[int, float]]) -> Dict[str, Any]:
        """Analyze trends in metric values."""        if len(values) < 3:
    ...
    async def _detect_anomalies(self, values: List[Union[int, float]]) -> List[Dict[str, Any]]:
        """Detect anomalies in metric values."""        if len(values) < 10:
            return []
    ...
    async def _calculate_percentiles(self, values: List[Union[int, float]]) -> Dict[str, float]:
        """Calculate percentiles for metric values."""        if not values:
            return {}
    ...
    async def _validate_metric_definition(self, definition: MetricDefinition) -> bool:
        """Validate metric definition."""        return bool(definition.metric_id and definition.name and definition.unit)
    
    async def _validate_alert_rule(self, rule: AlertRule) -> bool:
        """Validate alert rule."""        return bool(rule.rule_id and rule.metric_id and rule.condition and rule.message)
    
    async def _validate_dashboard(self, dashboard: Dashboard) -> bool:
        """Validate dashboard configuration."""        return bool(dashboard.dashboard_id and dashboard.name)
    ...
    async def get_metric_summary(self, metric_id: str) -> Optional[Dict[str, Any]]:
        """Get metric summary information."""        if metric_id not in self.metric_definitions:
            return None
    ...
    async def get_collector_stats(self) -> Dict[str, Any]:
        """Get metrics collector statistics."""        return {
            **self.collector_stats,
