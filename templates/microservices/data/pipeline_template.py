...
Data Pipeline Templates for iacherie Platform
==========================================
...
class DataSource(BaseModel):
    ...
    source_type: str = Field(..., pattern="^(database|file|api|stream|queue)$")
...
class DataTransformation(BaseModel):
    ...
    transformation_type: str = Field(..., pattern="^(filter|map|aggregate|join|pivot|sort)$")
...
class DataDestination(BaseModel):
    ...
    destination_type: str = Field(..., pattern="^(database|file|api|stream|cache)$")
    connection_config: Dict[str, Any] = Field(default_factory=dict)
...
def create_data_pipeline_app() -> FastAPI:
    ...
    """
    app = FastAPI(
        title="iacherie Data Pipeline Service",
