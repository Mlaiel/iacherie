# Privacy Policy for IA Chérie Mobile Application

**Effective Date: January 1, 2025**
**Last Updated: January 1, 2025**

## 1. Introduction

Welcome to IA Chérie ("we," "our," or "us"). This Privacy Policy explains how we collect, use, share, and protect your information when you use our mobile application ("App") and related services ("Services").

**Developer:** Fahed Mlaiel  
**Contact:** mlaiel@live.de  
**Website:** https://iacherie.com

## 2. Information We Collect

### 2.1 Information You Provide
- **Account Information:** Email address, username, profile information
- **Content:** Videos, images, audio files, and other content you create or upload
- **Communication:** Messages you send through customer support
- **Payment Information:** Processed securely through third-party payment processors

### 2.2 Automatically Collected Information
- **Device Information:** Device type, operating system, unique device identifiers
- **Usage Data:** How you interact with the App, features used, time spent
- **Technical Data:** IP address, app version, crash reports, performance metrics
- **Location Data:** Approximate location (only if you grant permission)

### 2.3 Content and Metadata
- **Created Content:** Videos, images, audio files created within the App
- **Metadata:** File information, creation timestamps, editing history
- **AI Analysis Data:** Content fingerprints for protection (processed locally when possible)

## 3. How We Use Your Information

### 3.1 Core App Functionality
- Providing content creation and editing tools
- Enabling content protection and monitoring
- Facilitating multi-platform publishing
- Generating analytics and insights
- Enabling collaboration features

### 3.2 Service Improvement
- Analyzing usage patterns to improve the App
- Debugging and fixing technical issues
- Developing new features and functionality
- Personalizing your experience

### 3.3 Communication
- Sending important service updates
- Responding to support requests
- Notifying about security issues
- Marketing communications (with consent)

### 3.4 Legal and Security
- Preventing fraud and abuse
- Complying with legal obligations
- Protecting intellectual property rights
- Enforcing our Terms of Service

## 4. Information Sharing

### 4.1 Third-Party Services
We may share information with:
- **Cloud Storage Providers:** For secure content backup
- **Analytics Services:** For app performance monitoring
- **Payment Processors:** For subscription and purchase processing
- **Content Delivery Networks:** For efficient content distribution

### 4.2 Platform Integration
- **Social Media Platforms:** When you choose to publish content
- **Content Monitoring Services:** For copyright protection
- **API Partners:** For enhanced functionality (with your consent)

### 4.3 Legal Requirements
We may disclose information if required by law, court order, or to:
- Protect our rights or property
- Investigate fraud or security issues
- Protect user safety
- Comply with regulatory requests

### 4.4 Business Transfers
In case of merger, acquisition, or sale, your information may be transferred to the new entity.

## 5. Data Security

### 5.1 Security Measures
- End-to-end encryption for sensitive content
- Secure data transmission (TLS/SSL)
- Regular security audits and updates
- Access controls and authentication
- Biometric security options (Face ID, Touch ID, Fingerprint)

### 5.2 Content Protection
- Local AI processing when possible
- Encrypted cloud storage
- Secure content fingerprinting
- Automated monitoring systems

### 5.3 Data Retention
- Account data: Retained while account is active
- Content: Retained per your settings or legal requirements
- Analytics data: Aggregated and anonymized after 2 years
- Backup data: Deleted after account termination (90 days)

## 6. Your Rights and Choices

### 6.1 Account Control
- Access and update your account information
- Delete your account and associated data
- Export your content and data
- Manage privacy settings

### 6.2 Content Control
- Choose what content to share or publish
- Set content visibility preferences
- Enable or disable content monitoring
- Control backup and sync settings

### 6.3 Communication Preferences
- Opt out of marketing communications
- Choose notification types and frequency
- Update contact preferences

### 6.4 Data Subject Rights (GDPR/CCPA)
If applicable, you may:
- Request access to your personal data
- Request correction of inaccurate data
- Request deletion of your data
- Request data portability
- Object to processing
- Withdraw consent

## 7. Children's Privacy

IA Chérie is not intended for children under 13. We do not knowingly collect personal information from children under 13. If we become aware that we have collected such information, we will delete it promptly.

## 8. International Data Transfers

Your information may be processed in countries other than your own. We ensure appropriate safeguards are in place for international transfers, including:
- Standard Contractual Clauses
- Adequacy decisions
- Certification mechanisms

## 9. California Privacy Rights (CCPA)

California residents have additional rights:
- Right to know what personal information is collected
- Right to delete personal information
- Right to opt-out of sale (we do not sell personal information)
- Right to non-discrimination

To exercise these rights, contact us at mlaiel@live.de.

## 10. European Privacy Rights (GDPR)

EU residents have rights under GDPR including:
- Lawful basis for processing
- Data Protection Officer contact: dpo@iacherie.com
- Right to lodge complaints with supervisory authorities

## 11. Cookies and Tracking

The App may use local storage and similar technologies to:
- Remember your preferences
- Analyze app performance
- Provide personalized features
- Enable essential functionality

You can manage these through your device settings.

## 12. Third-Party Links

Our App may contain links to third-party websites or services. This Privacy Policy does not apply to those third parties. Please review their privacy policies.

## 13. Updates to This Policy

We may update this Privacy Policy to reflect changes in our practices or legal requirements. We will notify you of material changes through:
- In-app notifications
- Email notifications
- App Store update notes

## 14. Contact Information

For privacy-related questions or concerns:

**Developer:** Fahed Mlaiel  
**Email:** mlaiel@live.de  
**Privacy Officer:** privacy@iacherie.com  
**Support:** support@iacherie.com  
**Address:** [Company Address]  

**EU Representative:** [EU Representative Details]  
**UK Representative:** [UK Representative Details]

## 15. Data Protection Officer

For GDPR-related inquiries:  
**Email:** dpo@iacherie.com

## 16. Supervisory Authority

EU users can contact their local supervisory authority for data protection concerns.

---

**This Privacy Policy is effective as of January 1, 2025, and governs your use of the IA Chérie mobile application developed by Fahed Mlaiel.**

By using our App, you acknowledge that you have read and understood this Privacy Policy.

---

*© 2025 Fahed Mlaiel. All rights reserved.*