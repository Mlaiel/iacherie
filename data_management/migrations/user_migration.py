...
class UserMigrationConfig:
    """Configuration for user migration operations"""
    migrate_passwords: bool = True
...
class UserMigration(BaseMigration):
    ...
    async def _migrate_user_profiles(self, session: Session):
        ...
        creator_profile_sql = """
        INSERT INTO creator_profiles (user_id, creator_tier)
        SELECT id, 'free'
        FROM users_enhanced
...
class CreatorMigration(UserMigration):
    ...
    async def _create_creator_enhancements(self, session: Session):
        """Create creator-specific table enhancements"""
        creator_enhancements = """
