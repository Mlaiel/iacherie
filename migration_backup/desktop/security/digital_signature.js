/**
 * IA Chéries Desktop - Digital Signature Management
 * 
...
class DigitalSignatureManager {
  ...
  async setupDefaultCA() {
    // Setup IA Chéries Certificate Authority
    ...
    if (!this.certificates.has(caId)) {
      ...
      console.log('🏛️ Created IA Chéries Certificate Authority');
    }
  }
  ...
  async createCertificateAuthority() {
    ...
    const certificate = {
      id: 'ainflue-ca',
      type: 'CA',
      subject: 'CN=IA Chéries Certificate Authority, O=IA Chéries, C=DE',
      issuer: 'CN=IA Chéries Certificate Authority, O=IA Chéries, C=DE',
  ...
  async createUserCertificate(userInfo) {
    ...
    const certificate = {
      id: certificateId,
      type: 'User',
      subject: `CN=${userInfo.name}, O=${userInfo.organization || 'IA Chéries'}, C=${userInfo.country || 'DE'}`,
      issuer: ca.subject,
      publicKey: keyPair.publicKey,
      serialNumber: this.generateSerialNumber(),
      notBefore: new Date(),
