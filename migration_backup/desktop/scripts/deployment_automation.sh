...
show_header() {
    echo -e "${PURPLE}${BOLD}"
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║                  🚀 IA CHÉRIES DEPLOYMENT AUTOMATION               ║"
    echo "║                                                                  ║"
    ...
    cat > "$update_config" << EOF
{
  ...
  "platforms": {
    "win32": {
      "url": "https://releases.ainflue.com/desktop/v${version}/IA Chéries-Setup-${version}.exe",
      "sha256": "$(sha256sum "${BUILD_DIR}/"*Setup*.exe 2>/dev/null | cut -d' ' -f1 || echo 'TBD')"
    },
    "darwin": {
      "url": "https://releases.ainflue.com/desktop/v${version}/IA Chéries-${version}.dmg",
      "sha256": "$(sha256sum "${BUILD_DIR}/"*.dmg 2>/dev/null | cut -d' ' -f1 || echo 'TBD')"
    },
    "linux": {
      "url": "https://releases.ainflue.com/desktop/v${version}/IA Chéries-${version}.AppImage",
...
main() {
    ...
    log "INFO" "🚀 Starting deployment for IA Chéries Desktop v${version}"
    ...
    echo "║                                                                  ║"
    echo "║  IA Chéries Desktop v${version} built for: $TARGET_PLATFORM        ║"
