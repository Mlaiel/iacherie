...
export default function PWAManager() {
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (e: Event) => {
      ...
      toast.success('📱 IA Chéries can be installed as an app!', {
        duration: 5000,
  ...
  const installPWA = async () => {
    ...
    try {
      ...
      if (choiceResult.outcome === 'accepted') {
        toast.success('🎉 IA Chéries installed successfully!');
        console.log('✅ PWA installation accepted');
      } else {
        toast('📱 You can install IA Chéries later from the browser menu');
    ...
    return (
      <div className="fixed bottom-4 right-4 z-50">
        <div className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-3">
          <span className="text-sm">Install IA Chéries as an app</span>
          <button
            onClick={installPWA}
