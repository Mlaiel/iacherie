...
class KeywordResearchRequest(BaseModel):
    ...
    target_language: str = Field("en", pattern="^[a-z]{2}$")
    target_country: str = Field("US", pattern="^[A-Z]{2}$")
...
class RankTrackingRequest(BaseModel):
    ...
    search_engine: SearchEngine = SearchEngine.GOOGLE
    location: str = Field("US", pattern="^[A-Z]{2}$")
    device_type: str = Field("desktop", pattern="^(desktop|mobile)$")
