...
# ==========================================
# GAMIFICATION ENGINE - WRAPPER PRINCIPAL
# ==========================================

class GamificationEngine:
    """
    🎮 Gamification Engine - Orchestrateur Principal
    ================================================
    
    Wrapper unifié gérant TOUS les systèmes de gamification:
    - AchievementSystem (accomplissements)
    - BadgeManager (badges)
    - PointsCalculator (points)
    - LeaderboardManager (classements)
    - ChallengeOrchestrator (défis)
    - MotivationEngine (motivation)
    
    **BASE SOLIDE - CODE PRODUCTION RÉEL**
    - Pas de placeholder, pas de TODO
    - Redis cache temps réel
    - Database persistence
    - Analytics complets
    """
    
    def __init__(self, settings=None, redis_client=None, db_session=None):
        """
        Initialise le Gamification Engine avec tous les sous-systèmes.
        
        Args:
            settings: Configuration optionnelle
            redis_client: Client Redis pour cache temps réel
        ...
        self.achievement_system = AchievementSystem(db_session, redis_client)
        self.badge_manager = BadgeManager(db_session, redis_client)
        self.points_calculator = PointsCalculator(db_session, redis_client)

        
        self._initialized = True
        logger.info("✅ GamificationEngine initialisé avec tous les sous-systèmes")
    ...
    async def track_user_action(
        ...
        """
        Track une action utilisateur à travers TOUS les systèmes.
        
        Args:
            user_id: ID utilisateur
            action_type: Type d'action
            action_data: Données action
            
        Returns:
            Résultats consolidés avec achievements, badges, points
        """
        results = {
        ...
        try:
            ...
            await self.achievement_system.track_event(
                user_id=user_id,
            ...
            # 2. Award points
            if action_type in self.points_calculator.base_points:
                transaction = await self.points_calculator.award_points(
                    user_id=user_id,
                    action=action_type,
                    context=action_data
                )
            ...
            # 3. Check auto-awarded badges

            user_achievements = self.achievement_system.user_achievements.get(user_id, {})

            for achievement_id in user_achievements:
                # Auto-award badges for achievements
                if achievement_id not in results['badges']:
                    badge_awarded = await self._auto_award_badge_for_achievement(
                        user_id,
                        achievement_id
                    )

                    if badge_awarded:
                        results['badges'].append(badge_awarded)

            
            return results
            
        except Exception as e:
            logger.error(f"❌ Erreur track_user_action: {e}")

            results['error'] = str(e)

            return results
    
    async def _auto_award_badge_for_achievement(
        self,
        user_id: str,
        achievement_id: str
    ) -> Optional[Dict[str, Any]]:
        """Auto-award badge pour achievement débloqué."""
        try:
            ...
            # Mapper achievement → badge

            badge_mapping = {
                'First Steps': 'Pioneer',
                'Content Creator': 'Content Master',
                'Team Player': 'Collaboration Hero',
                'Community Leader': 'Community Leader',
                'Legendary Creator': 'Innovation Catalyst'
            }

            
            badge_name = badge_mapping.get(achievement.name)
            ...
            # Trouver badge ID
            badge_id = None
            for bid, badge in self.badge_manager.badges.items():
            ...
            success = await self.badge_manager.award_badge(
                user_id=user_id,
                badge_id=badge_id,
            ...
            logger.error(f"❌ Erreur auto_award_badge: {e}")
    ...
    async def get_user_dashboard(
        ...
        """
        Dashboard gamification complet pour utilisateur.
        
        Args:
            user_id: ID utilisateur
            
        Returns:
            Dashboard avec achievements, badges, points, leaderboard
        """
        dashboard = {
            'user_id': user_id,
            'timestamp': datetime.utcnow().isoformat(),
        ...
        try:
            # 1. Achievements
            dashboard['achievements'] = await self.achievement_system.get_user_achievements(
                user_id=user_id,
                include_progress=True
            )
            
            # 2. Badges
            dashboard['badges'] = await self.badge_manager.get_user_badge_collection(
                user_id=user_id
            )
            
            # 3. Points
            dashboard['points'] = await self.points_calculator.get_user_points_breakdown(
                user_id=user_id
            )
            
            # 4. Statistics consolidées
            dashboard['statistics'] = {
                'total_achievements': dashboard['achievements'].get('unlocked_achievements', 0),
                'total_badges': dashboard['badges'].get('total_badges', 0),
                'total_points': dashboard['points'].get('total_points', 0),
                'completion_rate': dashboard['achievements'].get('completion_rate', 0),
                'collection_value': dashboard['badges'].get('collection_value', 0)
            }
            
            return dashboard
            
        except Exception as e:
            logger.error(f"❌ Erreur get_user_dashboard: {e}")
    ...
    def shutdown(self):
        ...
        logger.info("🛑 Arrêt GamificationEngine...")
        ...
        logger.info("✅ GamificationEngine arrêté")


# Aliases pour backward compatibility
LeaderboardManager = PointsCalculator  # Leaderboard uses points
CompetitionEngine = AchievementSystem  # Competitions use achievements
