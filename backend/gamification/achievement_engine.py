...
@dataclass
class GamingAchievement(Achievement):
    """Gaming-specific achievement with extended gaming features."""
    tycoon_level_requirement: Optional[int] = None
    wealth_threshold: Optional[Decimal] = None
    asset_count_requirement: Optional[int] = None
    efficiency_score_requirement: Optional[float] = None
    competitive_rank_requirement: Optional[int] = None
    gaming_category: str = "tycoon_mastery"
    leaderboard_position: Optional[int] = None
    tournament_id: Optional[str] = None
    power_up_rewards: List[str] = field(default_factory=list)
    exclusive_assets: List[str] = field(default_factory=list)
    multiplier_boost: float = 1.0
    
    def __post_init__(self):
        """Automatically flag as gaming achievement."""
        self.is_gaming = True
        if not self.category:
            self.category = AchievementCategory.TYCOON_MASTERY


class GamingAchievementType(str, Enum):
    """Gaming-specific achievement types."""
    TYCOON_MILESTONE = "tycoon_milestone"
    WEALTH_BUILDER = "wealth_builder"
    ASSET_MASTER = "asset_master"
    EFFICIENCY_CHAMPION = "efficiency_champion"
    COMPETITIVE_LEGEND = "competitive_legend"
    SPEED_RUNNER = "speed_runner"
    COLLECTION_COMPLETE = "collection_complete"


@dataclass
class GamingMilestone:
    """Gaming milestone tracking."""
    milestone_id: str = field(default_factory=lambda: str(uuid4()))
    player_id: str = ""
    milestone_type: GamingAchievementType = GamingAchievementType.TYCOON_MILESTONE
    threshold_value: Union[int, float, Decimal] = 0
...
async def track_metric(user_id: str, metric_name: str, value: Union[int, float], metadata: Optional[Dict[str, Any]] = None) -> List[Achievement]:
    """
    Track a specific metric for achievement progress.
    
    Args:
        user_id: User identifier
        metric_name: Name of the metric (e.g., 'videos_uploaded', 'collaborations_completed')
    ...
    return await engine.track_user_action(user_id, metric_name, metrics)
...
async def get_user_achievement_summary(user_id: str) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with total achievements, completion rate, points, etc.
    """
    engine = get_achievement_engine()
    achievements_data = await engine.get_user_achievements(user_id, include_hidden=False)
    stats = engine.user_stats.get(user_id)
    ...
    return {
        ...
        "completion_rate": (stats.unlocked_achievements / stats.total_achievements * 100) if stats.total_achievements > 0 else 0,
...
async def get_gaming_achievements(user_id: str, category: Optional[str] = None) -> List[GamingAchievement]:
    """
    Get gaming-specific achievements for user.
    
    Args:
        user_id: User identifier
        category: Optional category filter (e.g., 'tycoon_mastery')
    
    Returns:
        List of gaming achievements
    """
    engine = get_achievement_engine()
    all_achievements = await engine.get_user_achievements(user_id, include_hidden=False)
    
    gaming_achievements = []
    for ach_data in all_achievements.get("achievements", []):
        if ach_data.get("is_gaming", False):
            if category is None or ach_data.get("category") == category:
                # Convert to GamingAchievement
                gaming_ach = GamingAchievement(
                    achievement_id=ach_data.get("achievement_id", ""),
                    name=ach_data.get("name", ""),
                    title=ach_data.get("title", ""),
                    description=ach_data.get("description", ""),
                    gaming_category=ach_data.get("category", "tycoon_mastery")
                )
                gaming_achievements.append(gaming_ach)
    
    return gaming_achievements
