...
Complete business logic core providing comprehensive workflow management,
AI agent integration, and enterprise-level business process automation.
...
from .ia_agents_orchestrator import_business_core = None

async def get_business_core() -> BusinessLogicCore:
    """Get the singleton business logic core instance"""
    global _business_core
    if _business_core is None:
        _business_core = BusinessLogicCore()
        await _business_core.initialize()
    return _business_coreestrator, AIAgentsOrchestrator
...
class BusinessLogicCore:
    """
    Business Logic Core with 53 AI Agents Integration
