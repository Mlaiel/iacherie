...
class RateLimitType(str, Enum):
    """Rate limit type enumeration."""    REQUESTS_PER_SECOND = "requests_per_second"
...
class RateLimitConfig:
    """Rate limit configuration structure."""    platform: str
...
class RateLimitStatus:
    """Current rate limit status."""    platform: str
    current_usage: int
    limit_value: int
    remaining: int
    reset_time: datetime
...
class RequestMetrics:
    """Request performance metrics."""    timestamp: datetime
    platform: str
    priority: Priority
    response_time: float
    success: bool
...
class SlidingWindowCounter:
    ...
    def __init__(self, window_size: int, limit: int):
        """Initialize sliding window counter."""        self.window_size = window_size
        self.limit = limit
        self.requests: deque = deque()
        
    def add_request(self, timestamp: float = None) -> bool:
        """Add request and check if within limits."""        if timestamp is None:
            timestamp = time.time()
    ...
    def get_current_count(self) -> int:
        """Get current request count in window."""        current_time = time.time()
    ...
    def get_reset_time(self) -> datetime:
        """Get time when oldest request will expire."""        if not self.requests:
...
class TokenBucket:
    ...
    def __init__(self, capacity: int, refill_rate: float):
        """Initialize token bucket."""        self.capacity = capacity
        self.refill_rate = refill_rate  # tokens per second
        self.tokens = capacity
        self.last_refill = time.time()
        
    def consume(self, tokens: int = 1) -> bool:
        """Consume tokens and check availability."""        self._refill()
    ...
    def _refill(self):
        """Refill tokens based on time elapsed."""        now = time.time()
    ...
    def get_available_tokens(self) -> int:
        """Get number of available tokens."""        self._refill()
        return int(self.tokens)
    
    def get_wait_time(self, tokens: int = 1) -> float:
        """Get wait time until enough tokens are available."""        self._refill()
...
class AdaptiveRateLimiter:
    ...
    def __init__(self, base_config: RateLimitConfig):
        """Initialize adaptive rate limiter."""        self.base_config = base_config
    ...
    def record_performance(self, response_time: float, success: bool):
        """Record request performance for adaptation."""        self.performance_history.append({
            'timestamp': time.time(),
    ...
    def _adjust_rate_limit(self):
        """Adjust rate limit based on recent performance."""        if len(self.performance_history) < 10:
            return
    ...
    def get_current_limit(self) -> int:
        """Get current adjusted limit."""        return self.current_limit

class PriorityQueue:
    """Priority-based request queue."""    
    def __init__(self):
        """Initialize priority queue."""        self.queues: Dict[Priority, deque] = {
            priority: deque() for priority in Priority
        }
        self.pending_count = 0
        
    async def put(self, item: Any, priority: Priority = Priority.MEDIUM):
        """Add item to priority queue."""        self.queues[priority].append({
            'item': item,
            'timestamp': time.time(),
            'priority': priority
        })
    ...
    async def get(self) -> Optional[Tuple[Any, Priority]]:
        """Get highest priority item from queue."""        # Check queues in priority order
    ...
    def size(self) -> int:
        """Get total queue size."""        return self.pending_count
    
    def size_by_priority(self) -> Dict[Priority, int]:
        """Get queue size by priority."""        return {priority: len(queue) for priority, queue in self.queues.items()}
...
class IntelligentRateLimiter:
    ...
    """    
    def __init__(self):
        """Initialize intelligent rate limiter."""        self.platform_configs: Dict[str, RateLimitConfig] = {}
    ...
    def configure_platform(self, config: RateLimitConfig):
        """Configure rate limiting for platform."""        self.platform_configs[config.platform] = config
    ...
    async def record_request(
        ...
        """Record completed request for analytics and adaptation."""        # Record metrics
    ...
    async def _start_queue_processor(self):
        """Start background queue processor."""        if self._queue_processor_task and not self._queue_processor_task.done():
            return
    ...
    async def _process_queue(self):
        """Process queued requests with rate limiting."""        logger.info("Started rate limited queue processor")
    ...
    def _calculate_wait_time(self, platform: str) -> float:
        """Calculate wait time until next request can be made."""        if platform not in self.rate_limiters:
            return 0.0
    ...
    def _update_status(self, platform: str, can_proceed: bool):
        """Update platform rate limit status."""        if platform not in self.status_cache:
            return
    ...
    async def _check_alert_conditions(self, platform: str, metrics: RequestMetrics):
        """Check for alert conditions and trigger callbacks."""        # Check high error rate
    ...
    async def _trigger_alert(self, alert_type: str, data: Dict[str, Any]):
        """Trigger alert callbacks."""        alert_data = {
            'type': alert_type,
            'timestamp': datetime.utcnow(),
            'data': data
        }
    ...
    def register_alert_callback(self, callback: Callable):
        """Register callback for rate limiting alerts."""        self.alert_callbacks.append(callback)
    
    def get_platform_status(self, platform: str) -> Optional[RateLimitStatus]:
        """Get current rate limit status for platform."""        return self.status_cache.get(platform)
    
    def get_all_status(self) -> Dict[str, RateLimitStatus]:
        """Get rate limit status for all platforms."""        return self.status_cache.copy()
    ...
    def get_performance_metrics(self, platform: str = None) -> Dict[str, Any]:
        """Get performance metrics."""        if platform:
    ...
    async def shutdown(self):
        """Shutdown rate limiter and cleanup resources."""        self._processing_active = False
