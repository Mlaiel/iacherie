...
class CookieJar:
    ...
    def get_cookies_for_domain(self, domain: str, path: str = "/") -> List[CookieData]:
        """Get all valid cookies for domain and path."""
        valid_cookies = []
    ...
    def get_cookie_header(self, domain: str, path: str = "/") -> str:
        """Get cookie header string for domain."""
        cookies = self.get_cookies_for_domain(domain, path)
...
class CookieManager:
    ...
    def _should_encrypt_cookie(self, cookie: CookieData) -> bool:
        """Determine if cookie should be encrypted."""
        # Encrypt session tokens, auth cookies, etc.
        sensitive_patterns = [
            'session',
            'auth',
            'token',
    ...
    async def cleanup_expired_cookies(self) -> int:
        """Clean up expired cookies and return count removed."""
        initial_count = self.cookie_jar.get_cookie_count()
    ...
    async def get_cookie_statistics(self) -> Dict:
        ...
        domains = self.cookie_jar.get_domains()
