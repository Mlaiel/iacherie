...
class CacheDecision(Enum):
    """Cache operation decisions."""    CACHE = "cache"
    NO_CACHE = "no_cache"
    EVICT = "evict"
    COMPRESS = "compress"
    ENCRYPT = "encrypt"
...
@dataclass
class CacheEntry:
    """Cache entry with strategy metadata."""    key: str
    value: Any
    created_at: datetime
    ...
    def is_expired(self) -> bool:
        """Check if entry is expired."""        if self.ttl_seconds is None:
            return False
        expires_at = self.created_at + timedelta(seconds=self.ttl_seconds)
        return datetime.now() > expires_at
    
    def age_seconds(self) -> float:
        """Get entry age in seconds."""        return (datetime.now() - self.created_at).total_seconds()
    
    def idle_seconds(self) -> float:
        """Get idle time since last access."""        return (datetime.now() - self.last_accessed).total_seconds()

@dataclass
class StrategyMetrics:
    """Strategy performance metrics."""    hit_rate: float = 0.0
    miss_rate: float = 0.0
...
class CacheStrategy:
    ...
    def __init__(self, strategy_type: StrategyType, config: Optional[Dict[str, Any]] = None):
        """Initialize cache strategy."""        self.strategy_type = strategy_type
    ...
    async def should_evict(self, key: str) -> bool:
        """Decide whether to evict a specific entry."""        entry = self.entries.get(key)
        if not entry:
            return False
    ...
    async def record_access(self, key: str) -> None:
        """Record cache access for strategy optimization."""        self.total_requests += 1
    ...
    async def add_entry(self, key: str, value: Any, 
        ...
        """Add entry to cache with strategy tracking."""        try:
            entry = CacheEntry(
                key=key,
                value=value,
    ...
    async def remove_entry(self, key: str) -> bool:
        """Remove entry from cache and tracking."""        try:
            if key in self.entries:
    ...
    def _estimate_size(self, value: Any) -> int:
        """Estimate memory size of value."""        if isinstance(value, str):
    ...
    async def _strategy_should_evict(self, entry: CacheEntry) -> bool:
        """Strategy-specific eviction decision."""        # Base implementation - override in subclasses
        return False
    
    async def _strategy_select_candidates(self, count: int) -> List[str]:
        """Strategy-specific candidate selection."""        # Base implementation - LRU
        return self.access_order[:count]
    
    async def _update_metrics(self) -> None:
        """Update strategy metrics."""        if self.total_requests > 0:
    ...
    async def get_metrics(self) -> StrategyMetrics:
        """Get current strategy metrics."""        await self._update_metrics()
        return self.metrics
    
    async def optimize(self) -> Dict[str, Any]:
        """Optimize strategy based on current performance."""        # Base optimization - override in subclasses
...
class AdaptiveStrategy(CacheStrategy):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize adaptive strategy."""        super().__init__(StrategyType.ADAPTIVE, config)
    ...
    async def should_cache(self, key: str, value: Any,
                          metadata: Optional[Dict[str, Any]] = None) -> CacheDecision:
        """Adaptive caching decision."""        # Use weighted combination of strategies
        scores = {}
    ...
    async def select_eviction_candidates(self, count: int = 1) -> List[str]:
        """Adaptive eviction candidate selection."""        if not self.entries:
            return []
    ...
    async def record_access(self, key: str) -> None:
        """Record access and potentially adapt strategy."""        await super().record_access(key)
    ...
    async def _adapt_strategy(self) -> None:
        """Adapt strategy weights based on performance."""        try:
    ...
    async def _lru_score(self, key: str, value: Any, 
        ...
        """LRU-based scoring."""        if key in self.entries:
            entry = self.entries[key]
    ...
    async def _lfu_score(self, key: str, value: Any,
                        metadata: Optional[Dict[str, Any]] = None) -> float:
        """LFU-based scoring."""        if key in self.entries:
            entry = self.entries[key]
    ...
    async def _ttl_score(self, key: str, value: Any,
                        metadata: Optional[Dict[str, Any]] = None) -> float:
        """TTL-based scoring."""        if key in self.entries:
            entry = self.entries[key]
    ...
    async def _size_score(self, key: str, value: Any,
                         metadata: Optional[Dict[str, Any]] = None) -> float:
        """Size-based scoring."""        size = self._estimate_size(value)
        # Prefer smaller items (inverse scoring)
        max_size = 1048576  # 1MB
        return max(0, 1 - size / max_size)
    
    async def optimize(self) -> Dict[str, Any]:
        """Optimize adaptive strategy."""        optimizations = []
...
def create_strategy(strategy_type: StrategyType, 
    ...
    """Create cache strategy instance."""    if strategy_type == StrategyType.ADAPTIVE:
