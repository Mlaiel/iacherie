...
@dataclass
class ProxyInfo:
    """Proxy server information."""    host: str
    port: int
...
class ProxyScraper:
    ...
    def add_proxy(self, proxy: ProxyInfo):
        """Add proxy to pool."""        self.proxy_pool.append(proxy)
    ...
    def add_proxies_from_list(self, proxy_list: List[str]):
        """Add proxies from string list (host:port format)."""        for proxy_str in proxy_list:
            try:
                if ':' in proxy_str:
    ...
    async def test_proxy(self, proxy: ProxyInfo) -> bool:
        """Test if proxy is working."""        proxy_url = self._format_proxy_url(proxy)
    ...
    async def test_all_proxies(self):
        """Test all proxies in pool."""        self.logger.info("Testing all proxies...")
    ...
    def get_next_proxy(self) -> Optional[ProxyInfo]:
        """Get next proxy using rotation strategy."""        if not self.proxy_pool:
            return None
    ...
    def get_best_proxy(self) -> Optional[ProxyInfo]:
        """Get proxy with best performance."""        working_proxies = [p for p in self.proxy_pool if p.is_working]
    ...
    def get_random_proxy(self) -> Optional[ProxyInfo]:
        """Get random working proxy."""        working_proxies = [p for p in self.proxy_pool if p.is_working]
    ...
    def _format_proxy_url(self, proxy: ProxyInfo) -> str:
        """Format proxy URL for aiohttp."""        url = f"{proxy.protocol}://"
    ...
    async def make_request_with_proxy(self, url: str, proxy: Optional[ProxyInfo] = None,
        ...
        """Make HTTP request using proxy."""        if not proxy:
    ...
    def remove_dead_proxies(self):
        """Remove proxies that are consistently failing."""        initial_count = len(self.proxy_pool)
    ...
    def get_proxy_stats(self) -> Dict[str, Any]:
        """Get proxy pool statistics."""        if not self.proxy_pool:
            return {}
    ...
    def export_working_proxies(self) -> List[str]:
        """Export working proxies as string list."""        working_proxies = [p for p in self.proxy_pool if p.is_working]
