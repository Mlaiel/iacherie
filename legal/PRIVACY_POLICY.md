# Privacy Policy - IA Chérie Platform

**Last Updated: January 2025**

## 1. Introduction

This Privacy Policy describes how Fahed Mlaiel and IA Chérie Platform ("Company", "we", "us", or "our") collects, uses, processes, and protects your personal information when you use our AI-powered content protection and creator monetization platform ("Service").

We are committed to protecting your privacy and ensuring transparency about our data practices. This policy complies with the General Data Protection Regulation (GDPR), California Consumer Privacy Act (CCPA), Lei Geral de Proteção de Dados (LGPD), and other applicable privacy laws.

## 2. Information We Collect

### 2.1 Personal Information
We collect the following types of personal information:

**Account Information:**
- Name, email address, phone number
- Username and password
- Profile picture and biographical information
- Payment and billing information
- Business information (company name, tax ID, address)

**Content Information:**
- Digital content files you upload for protection
- Metadata associated with your content
- Content ownership and licensing information
- Platform-specific content identifiers

**Usage Information:**
- Log data and analytics information
- Device information (IP address, browser type, operating system)
- Platform interaction data and preferences
- Feature usage patterns and service performance metrics

### 2.2 Technical Information
**AI Processing Data:**
- Content fingerprints and digital signatures
- AI-generated insights and recommendations
- Content matching and similarity analysis results
- Performance metrics and optimization data

**Integration Data:**
- Third-party platform API tokens (securely encrypted)
- Platform-specific user identifiers
- Content performance and engagement metrics
- Monetization and revenue tracking data

### 2.3 Automatically Collected Information
- Cookies and similar tracking technologies
- Session information and user preferences
- Security logs and access patterns
- Error reports and debugging information

## 3. How We Use Your Information

### 3.1 Service Provision
We use your information to:
- Provide content protection and monitoring services
- Generate AI-powered insights and recommendations
- Process DMCA takedown notices and legal claims
- Facilitate content monetization opportunities
- Maintain platform security and prevent fraud

### 3.2 Communication
- Send service-related notifications and updates
- Provide customer support and technical assistance
- Share important legal and policy updates
- Send marketing communications (with your consent)

### 3.3 Improvement and Analytics
- Analyze service usage and performance
- Develop and improve AI algorithms
- Conduct research and development
- Generate anonymized statistical reports

### 3.4 Legal Compliance
- Comply with legal obligations and regulatory requirements
- Respond to legal requests and enforce our Terms of Service
- Protect our rights and interests
- Prevent illegal activity and policy violations

## 4. Legal Basis for Processing (GDPR)

We process your personal information based on the following legal grounds:

### 4.1 Contractual Necessity
- Performing our contract to provide the Service
- Processing payments and managing subscriptions
- Providing customer support and technical assistance

### 4.2 Legitimate Interests
- Improving and developing our Service
- Ensuring platform security and preventing fraud
- Conducting business analytics and research
- Marketing our services to existing customers

### 4.3 Legal Obligations
- Complying with tax and accounting requirements
- Responding to legal requests and court orders
- Meeting regulatory compliance obligations

### 4.4 Consent
- Processing special categories of personal data
- Sending marketing communications
- Using cookies and tracking technologies
- Processing data for additional purposes not covered above

## 5. Information Sharing and Disclosure

### 5.1 Third-Party Service Providers
We may share your information with trusted service providers who assist us in:
- Cloud hosting and data storage (AWS, Google Cloud, Azure)
- Payment processing (Stripe, PayPal)
- Email and communication services (SendGrid, Mailchimp)
- Analytics and monitoring tools (Google Analytics, Mixpanel)
- Customer support platforms (Zendesk, Intercom)

### 5.2 Platform Integrations
With your explicit consent, we may share limited information with:
- Social media platforms (YouTube, Instagram, TikTok) for content monitoring
- Content management systems for protection services
- Legal service providers for DMCA processing

### 5.3 Legal Requirements
We may disclose your information when required by law:
- To comply with court orders, subpoenas, or legal processes
- To protect our rights, property, or safety
- To investigate potential violations of our Terms of Service
- To prevent fraud, security threats, or illegal activity

### 5.4 Business Transfers
In the event of a merger, acquisition, or sale of assets, your information may be transferred to the acquiring entity, subject to the same privacy protections.

## 6. International Data Transfers

### 6.1 Cross-Border Processing
We operate globally and may transfer your information across borders for processing. When we transfer personal data outside the European Economic Area (EEA), we ensure appropriate safeguards are in place:

### 6.2 Transfer Mechanisms
- Standard Contractual Clauses (SCCs) approved by the European Commission
- Adequacy decisions recognized by relevant data protection authorities
- Binding Corporate Rules for intra-group transfers
- Your explicit consent for specific transfers

### 6.3 Data Localization
For users in certain jurisdictions with data localization requirements:
- Brazilian data (LGPD compliance) is primarily processed within Brazil
- Russian data is processed in accordance with Federal Law No. 152-FZ
- Chinese data is handled in compliance with Cybersecurity Law requirements

## 7. Data Retention

### 7.1 Retention Periods
We retain your personal information for as long as necessary to:
- Provide the Service and fulfill our contractual obligations
- Comply with legal retention requirements
- Resolve disputes and enforce our agreements
- Pursue legitimate business interests

### 7.2 Specific Retention Periods
**Account Information:** Retained while your account is active, plus 7 years after closure for legal compliance

**Content Data:** Retained while you use our protection services, plus 30 days after service termination

**Usage Analytics:** Anonymized after 24 months, aggregated data retained indefinitely

**Financial Records:** Retained for 7 years in compliance with tax and accounting requirements

**Legal Communications:** Retained for 10 years for legal and compliance purposes

### 7.3 Deletion Process
When retention periods expire, we securely delete or anonymize your information using industry-standard methods.

## 8. Your Privacy Rights

### 8.1 Universal Rights
Regardless of your location, you have the right to:
- Access your personal information
- Request correction of inaccurate data
- Request deletion of your data (subject to legal obligations)
- Object to processing for direct marketing

### 8.2 GDPR Rights (EU/EEA Users)
Additional rights under GDPR include:
- **Right to Portability:** Receive your data in a machine-readable format
- **Right to Restriction:** Limit how we process your data
- **Right to Object:** Object to processing based on legitimate interests
- **Right to Withdraw Consent:** Withdraw consent for consent-based processing

### 8.3 CCPA Rights (California Users)
Under the California Consumer Privacy Act, you have the right to:
- Know what personal information we collect and how it's used
- Request deletion of your personal information
- Opt-out of the sale of personal information (we don't sell personal information)
- Non-discrimination for exercising your privacy rights

### 8.4 LGPD Rights (Brazilian Users)
Under Brazil's Lei Geral de Proteção de Dados, you have the right to:
- Confirmation of data processing
- Access to your personal data
- Correction of incomplete or inaccurate data
- Anonymization, blocking, or deletion of data
- Data portability to another service provider

## 9. Data Security

### 9.1 Security Measures
We implement comprehensive security measures to protect your information:

**Technical Safeguards:**
- End-to-end encryption for data in transit (TLS 1.3)
- AES-256 encryption for data at rest
- Multi-factor authentication for account access
- Regular security assessments and penetration testing

**Administrative Safeguards:**
- Employee privacy training and access controls
- Data processing agreements with service providers
- Incident response and breach notification procedures
- Regular privacy impact assessments

**Physical Safeguards:**
- Secure data centers with restricted access
- Environmental controls and monitoring
- Backup and disaster recovery procedures

### 9.2 Data Breach Response
In the event of a data breach, we will:
- Contain and assess the incident within 24 hours
- Notify relevant authorities within 72 hours (where required)
- Inform affected users without undue delay
- Provide regular updates on investigation and remediation efforts

## 10. Cookies and Tracking Technologies

### 10.1 Types of Cookies We Use
**Essential Cookies:** Required for basic service functionality
**Performance Cookies:** Help us analyze service usage and performance
**Functional Cookies:** Remember your preferences and settings
**Marketing Cookies:** Used for personalized advertising (with consent)

### 10.2 Cookie Management
You can control cookies through:
- Browser settings and preferences
- Our cookie consent management platform
- Third-party opt-out tools and preferences

### 10.3 Do Not Track
We respect "Do Not Track" signals where technically feasible and legally required.

## 11. Children's Privacy

Our Service is not intended for children under 16 years of age. We do not knowingly collect personal information from children under 16. If we become aware that we have collected personal information from a child under 16, we will take immediate steps to delete such information.

## 12. Third-Party Links and Services

Our Service may contain links to third-party websites and services. We are not responsible for the privacy practices of these third parties. We encourage you to review their privacy policies before providing personal information.

## 13. Changes to This Privacy Policy

### 13.1 Policy Updates
We may update this Privacy Policy to reflect changes in our practices or applicable laws. We will notify you of material changes through:
- Email notification to your registered address
- Prominent notice on our Service
- In-app notifications for mobile users

### 13.2 Continued Use
Your continued use of the Service after policy changes constitutes acceptance of the updated Privacy Policy.

## 14. Contact Information

### 14.1 Privacy Questions
For questions about this Privacy Policy or our privacy practices:

**Privacy Officer:**  
Email: privacy@iacherie.com  
Address: [To be provided based on legal entity establishment]

**Data Protection Officer (EU):**  
Email: dpo@iacherie.com

### 14.2 Exercising Your Rights
To exercise your privacy rights:
- Email: privacy-rights@iacherie.com
- Account Settings: Use the privacy controls in your account dashboard
- Phone: [To be provided]

### 14.3 Regulatory Authorities
You have the right to lodge a complaint with your local data protection authority if you believe we have violated your privacy rights.

**EU Supervisory Authorities:** https://edpb.europa.eu/about-edpb/board/members_en  
**California Attorney General:** https://oag.ca.gov/privacy/ccpa  
**Brazilian ANPD:** https://www.gov.br/anpd/pt-br

---

**Effective Date:** January 1, 2025  
**Version:** 1.0  
**Copyright:** © 2025 Fahed Mlaiel. All rights reserved.