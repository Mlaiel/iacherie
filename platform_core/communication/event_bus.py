...
class EventPriority(Enum):
    """Priorités des événements"""    LOW = 1
    NORMAL = 3
    HIGH = 5
    CRITICAL = 10

class EventStatus(Enum):
    """États des événements"""    PENDING = "pending"
    PUBLISHED = "published"
    DELIVERED = "delivered"
    FAILED = "failed"
...
@dataclass
class Event:
    """Événement du système"""    event_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    event_type: str = ""
    source: str = ""
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convertit l'événement en dictionnaire"""        data = asdict(self)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Event':
        """Crée un événement depuis un dictionnaire"""        # Convertir les dates
...
class EventSubscription:
    """Abonnement à des événements"""    subscription_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    subscriber_id: str = ""
    ...
    def matches_event(self, event: Event) -> bool:
        """Vérifie si l'événement correspond à l'abonnement"""        if not self.active:
            return False
    ...
    def _matches_pattern(self, pattern: str, event_type: str) -> bool:
        """Vérifie si un type d'événement correspond au pattern"""        if pattern == "*":
            return True
        if pattern.endswith("*"):
...
class EventHandler:
    ...
    async def handle(self, event: Event) -> bool:
        """Exécute le handler pour un événement"""        try:
            if self.is_async:
...
class EventBus:
    ...
    async def start(self):
        """Démarre le bus d'événements"""        self._running = True
    ...
    async def stop(self):
        """Arrête le bus d'événements"""        self._running = False
    ...
    async def unsubscribe(self, subscription_id: str):
        """Se désabonne d'événements"""        if subscription_id in self.subscriptions:
    ...
    async def _process_events(self):
        """Boucle de traitement des événements"""        while self._running:
            try:
    ...
    async def _process_event_message(self, stream_name: str, message_id: str, fields: Dict):
        """Traite un message d'événement"""        try:
    ...
    async def _distribute_event(self, event: Event) -> bool:
        """Distribue un événement aux abonnés correspondants"""        delivered = False
    ...
    async def _ack_message(self, message_id: str):
        """Acknowledge un message traité"""        try:
    ...
    async def _send_to_dlq(self, event: Event):
        """Envoie un événement en Dead Letter Queue"""        event.status = EventStatus.DEAD_LETTER
    ...
    async def replay_events(self, 
        ...
        """Rejoue des événements depuis l'historique"""        replayed = 0
    ...
    async def get_event_history(self, 
        ...
        """Récupère l'historique d'événements pour un agrégat"""        events = []
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Retourne les statistiques du bus d'événements"""        uptime = (datetime.utcnow() - self.start_time).total_seconds()
